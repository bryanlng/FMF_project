...
public class FMCLocationData{
			...
			boolean gps, boolean network, String wifi, String best, String loc1, String loc2)
	{
		...
		else if (wifi.contains("<"))
		{
			...
			int length = fullWifiMessage.length();
			if(length <= 255){
				System.out.println("fullWifiMessage length < 255, inside constructor");
				this.wifiAvailableSSIDs = a.split(",");
				//Wifi SSID = wife name
				//Ex: Puts ATT88,ADF,WWFWE ==> String array of available WIFIs
			}
			else{
				System.out.println("fullWifiMessage length > 255");
				/* If fullwifimessage > 255 
				 * 
				 * Problem here = Wifi column of non_location_fields oan only hold a maximum length of 255 characters
				 * Thus, if we have a string of greater length ==> exception
				 * 
				 * A. Choosing and truncating items to put into wifiAvailableSSIDsProcess:
				 * 	1. Put split string into a temporary String array
				 *  2. Check the length of each and add it on to the counter. Increment index too.
				 *  3. Once counter > 255 ==> exit loop
				 *  4. Put all strings up to (index - 1) from temp string array ==> finished string array
				 *  
				 * B. Reconstructing fullWifiMessage
				 *  1.Combine parts "WF:ECG <" + available SSIDs+ ">"
				 *  2.Use StringBuffer b/c it's faster
				 * 
				 */
				//Part A
				String[] beforeTruncate = a.split(",");
				//	                System.out.println("beforeTruncate:");		//debug commented out
				//	                for(String w: beforeTruncate){
				//	                	System.out.println(w);
				//	                }
				//	                System.out.println("");

				int index = 0;
				int totalLength = 8;	//b/c fullWifiMessage has stuff at the beginning. ex: WF:ECG <....>
				while(totalLength <= 254 && index < beforeTruncate.length){	//254 because we need to add a > at the end
					String temp = beforeTruncate[index];
					totalLength += temp.length() + 1;	//+ 1 for the comma after each one
					index++;
				}

				//since at the end, index++ and then it doesn't fulfill condition anymore,
				//the last good index was (index - 1)
				//Put all strings up to (index - 1) from beforeTruncate into afterTruncate
				String[] afterTruncate = new String[index - 1];
				for(int k = 0; k < afterTruncate.length; k++){
					afterTruncate[k] = beforeTruncate[k];
				}
				this.wifiAvailableSSIDs = afterTruncate;

				//	                System.out.println("afterTruncate:");		//debug commented out
				//	                for(String w: afterTruncate){
				//	                	System.out.println(w);
				//	                }
				//	                System.out.println("");
				//Part B

				//Create a new StringBuffer object with starting part being the string from 0-8, aka WF:ECG <
				StringBuffer sbuffer = new StringBuffer(getRawWifiMessage().substring(0,9));	
				for(String w: afterTruncate){
					sbuffer.append(w + ",");
				}
				sbuffer.delete(sbuffer.length() - 1, sbuffer.length());	//delete the last comma at the end
				sbuffer.append(">");	//add on the last > 

				//	               setRawWifiMessage(sbuffer.toString());
				//	               System.out.println("New full wifi message: " + sbuffer.toString() + "\n");	//debug commented out

				fullWifiMessage = sbuffer.toString();
			}
	...
	public void setRawWifiMessage(String s){
		fullWifiMessage = s;
	}
